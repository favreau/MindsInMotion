...
const moveParticles = gpu.createKernel(function(positions, velocities, noise, attractionDistance, width, height) {
  ...
  let vy = velocities[this.thread.x][1];
    ...
    return [x1, y1, vx, vy];
}).setOutput([numParticles]).setConstants({ numParticles, maxSpeed });

// GPU Kernel to handle particle interactions
const interactParticles = gpu.createKernel(function(positions, colors, originalColors, charges, influenceDistance, powerOfLove) {
  ...
  for (let i = 0; i < this.constants.numParticles; i++) {
    if (i !== this.thread.x) {
      ...
      if (dist < influenceDistance) {
        ...
        const influenceStrength = (influenceDistance - dist) / influenceDistance * powerOfLove;
  ...
  return [r, g, b];
}).setOutput([numParticles]).setConstants({ numParticles, recoverySpeed });
...
function draw() {
  ...
  fill(255);
  textSize(12);
  text('Random Movement    : ' + noiseSlider.value(), 10, uiVerticalOffset + 10);
  text('Power of Love      : ' + powerOfLoveSlider.value(), 10, uiVerticalOffset + 60);
  text('Attraction distance: ' + attractionDistanceSlider.value(), 10, uiVerticalOffset + 110);
  text('Influence distance : ' + influenceDistanceSlider.value(), 10, uiVerticalOffset + 160);
  ...
  const newColors = interactParticles(particlePositions, particleColors, particleOriginalColors, particleCharges, influenceDistance, powerOfLove);
